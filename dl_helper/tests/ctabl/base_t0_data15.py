...
class test(test_base):
    ...
    def __init__(self, *args, target_type=1, **kwargs):
        ...
        for predict_n in [3, 10, 30, 60, 100]:
            if predict_n == 100:
                ...
                for p in [
                    ...
                    vars.append((predict_n, classify_idx, p))
            classify_idx+=1
        ...
        label_idx = 4
        predict_n, classify_idx, self.model_p = vars[self.idx]
        ...
        title = self.title_base() + f"_predict_n{predict_n}_label_idx{label_idx}_model_{['mid', 'small'][self.idx]}"
        ...
        self.para = Params(
            ...
            describe=f"model {'@'.join(str(self.model_p))}",
            amp=self.amp
        )
    ...
    def get_model(self):
        ...
        # # T: 100, 30, 10, 1
        # # D: 44, 20, 10, 3
        return m_bin_ctabl(self.model_p[2], 44, 100,  self.model_p[0],  self.model_p[3],  self.model_p[1], self.y_n, 1)
