...
测试 历史数据 200/150/100(default)/80/40
"""
...
class test(test_base):
    ...
    def __init__(self, *args, target_type=1, **kwargs):
        ...
        for predict_n in [3, 30, 60, 100]:
            # 同一个训练使用4个随机种子，最终取均值
            if predict_n == 100:
                for pass_n in [200, 150, 80, 40]:
                    for seed in range(4):
                        vars.append((predict_n, classify_idx, seed, pass_n))
            classify_idx+=1

        predict_n, classify_idx, seed, self.pass_n = vars[self.idx]
        ...
        title = self.title_base() + f"{self.pass_n}_predict_n{predict_n}_seed{seed}"
    ...
    def get_in_out_shape(self):
        return (1, 21, self.pass_n), (1, self.y_n)

    # 初始化模型
    # 返回一个 torch model
    def get_model(self):
        # SMALL
        t1, t2, t3, t4 = [self.pass_n, 30, 10, 1]
        d1, d2, d3, d4 = [21, 20, 10, 3]
        return m_bin_ctabl(d2, d1, t1, t2, d3, t3, d4, t4)

    def get_transform(self, device):
        return transform_stable(device, self.para, 200, num_rows=21)
