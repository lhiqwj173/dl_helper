...
class test(test_base):
    ...
    @classmethod
    def title_base(cls):
        return f'once_of_label_1_multi_reg'
    ...
    def __init__(self, *args, target_type=1, **kwargs):
        ...
        for predict_n in [3, 5, 10, 15, 30, 60, 100]:
            for label in ['paper', 'paper_pct', 'label_1', 'label_1_pct']:
                if label == 'label_1':
                    regress_idxs.append(regress_idx)
                regress_idx+=1

        # 同一个训练使用 6 个随机种子，最终取均值
        seed = range(6)[self.idx]

        self.y_n = len(regress_idxs)
