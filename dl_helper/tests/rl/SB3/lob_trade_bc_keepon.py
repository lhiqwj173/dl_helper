...
if run_type != 'test':
    ...
    label_balance = f'训练样本均衡度: {cal_action_balance(transitions)} ({len(transitions)} 样本)'
    log(label_balance)
    msg += label_balance + '\n'
    label_balance = f'验证样本均衡度: {cal_action_balance(transitions_val)} ({len(transitions_val)} 样本)'
    log(label_balance)
    msg += label_balance + '\n'
    send_wx(msg)
    # sys.exit()
