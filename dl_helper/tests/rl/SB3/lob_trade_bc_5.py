...
class DeepLob(BaseFeaturesExtractor):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        x = use_obs[:, :-self.extra_input_dims].reshape(-1, 1, *self.input_dims)  # (B, 1, 100, 8)
        ...
        # 卷积块
        x = self.conv1(x)  # (B, 32, 98, 4)
        x = self.conv2(x)  # (B, 32, 96, 2)
        x = self.conv3(x)  # (B, 32, 94, 1)

        # Inception 模块
        x_inp1 = self.inp1(x)  # (B, 64, 94, 1)
        x_inp2 = self.inp2(x)  # (B, 64, 94, 1)
        x_inp3 = self.inp3(x)  # (B, 64, 94, 1)
        x = torch.cat((x_inp1, x_inp2, x_inp3), dim=1)  # (B, 192, 94, 1)
        x = self.dp2d(x)  # 添加 Dropout2d

        # 调整形状以适配 LSTM
        x = x.permute(0, 2, 1, 3).squeeze(3)  # (B, 94, 192)

        # LSTM 处理 取最后一个时间步
        lstm_out, _ = self.lstm(x)  # (B, 94, 64)
...
env_config ={
    ...
    'need_cols': [item for i in range(2) for item in [f'BASE卖{i+1}价', f'BASE卖{i+1}量', f'BASE买{i+1}价', f'BASE买{i+1}量']],
...
if run_type != 'test':
    ...
    if not in_windows():
        data_folder = [
            ...
            rf'/kaggle/input/lob-bc-train-data-filted-5/',
        ]
    else:
