    ...
    def step(self, action):
        ...
        if no_wall_collision and no_tail_collision and self.time < self.max_steps:
            ...
            if np.array_equal(new_pos, self.apple):
                ...
                if self.prev_apple_distance is None:
                    self.prev_apple_distance = apple_distance
                if self.prev_apple_distance > apple_distance:
                    self.reward += 1
                else:
                    self.reward -= 1
                self.prev_apple_distance = apple_distance
...
save_name = f'simple_snake4_{model_type}'
...
if run_type == 'train':
    ...
    # 生成模型实例    
    kaggle_keep_trained_file = rf'/kaggle/input/checkpoint/{save_name}.checkpoint'
    if os.path.exists(kaggle_keep_trained_file):
        print(f'加载模型, 继续训练')
        # 从kaggle保存的模型文件中拷贝到当前目录
        shutil.copy(kaggle_keep_trained_file, f'./{save_name}_checkpoint.zip')
        # load model
        model = PPO.load(f'./{save_name}_checkpoint.zip', env, device=device, custom_objects= {"policy_kwargs": policy_kwargs} if model_type == 'CnnPolicy' else None)
    else:
        print(f'新训练')
    ...
    model = PPO.load(
        rf"D:\code\dl_helper\dl_helper\tests\rl\SB3\{save_name}", 
