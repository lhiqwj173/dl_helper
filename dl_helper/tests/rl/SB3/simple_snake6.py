        ...
        if no_wall_collision and no_tail_collision and self.time < self.max_steps:
            ...
            if np.array_equal(new_pos, self.apple):
                ...
                if len(self.snake) < self.s ** 2:
                    self.reset_apple()
                    self.reward = len(self.snake) * 100  # 吃苹果的奖励
                    terminated = False
                # else:
...
if run_type == 'train':
    ...
    model.save(f"simple_snake6")
    
    # for i in range(1000):
    #     model.learn(total_timesteps=1000000)
    #     model.save(f"simple_snake6_{i}")
else:
    env = SnakeEnv({
        'render_mode': 'human',
    })
    model = PPO.load(r"D:\code\dl_helper\dl_helper\tests\rl\SB3\simple_snake6.zip")
