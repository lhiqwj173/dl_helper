...
class SnakeEnv(gym.Env):
    ...
    def state(self):
        # 创建一个 (10,10) 的通道，初始化为 0，数据类型为 float32
        grid = np.zeros(self.grid_size, dtype=np.float32)
        
        # 设置蛇身位置（不包括蛇头），值为 0.7
        for pos in self.snake[1:]:
            grid[pos[0], pos[1]] = 0.7
        
        # 设置蛇头位置，值为 1.0
        if len(self.snake):  # 确保蛇列表不为空
            head_pos = self.snake[0]
            grid[head_pos[0], head_pos[1]] = 1.0
        
        # 设置苹果位置，值为 0.3
        grid[self.apple[0], self.apple[1]] = 0.3
        
        # 增加一个维度，保持与常见深度学习输入格式兼容，返回形状 (1, 10, 10)
