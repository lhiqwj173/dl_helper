...
# 参数量: 32724
class LinearLob(BaseFeaturesExtractor):
    net_arch = [4, 4]  # 架构参数保持不变
    def __init__(
            self,
            observation_space,
            ...
            input_dims: tuple = (10, 20),  # 时间维度和特征维度
            extra_input_dims: int = 3,
            dropout_rate: float = 0.1,  # 假设 arg_dropout 默认值
    ):
        super().__init__(observation_space, features_dim)
        ...
        # 计算线性层输入维度 - 将时间序列数据展平
        # 输入形状为 [batch, time, features]，展平后为 [batch, time * features]
        self.linear_input_dim = input_dims[0] * input_dims[1]  # 时间维度 * 特征维度
        
        # 线性层
        self.linear_blocks = nn.Sequential(
            nn.Linear(self.linear_input_dim, hidden_dim * 2),
            nn.LeakyReLU(),
        ...
        # 融合网络
        fusion_dim = hidden_dim + hidden_dim  # 时序特征 + 静态特征
        self.fusion = nn.Sequential(
            nn.Linear(fusion_dim, fusion_dim),
            nn.LeakyReLU(),
            nn.Linear(fusion_dim, features_dim)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        batch_size = observations.size(0)
        time_dim, feature_dim = self.input_dims
        x = use_obs[:, :-self.extra_input_dims].reshape(batch_size, -1)  # 展平为 [batch, time * features]
        ...
        embedded = self.id_embedding(cat_feat)
        static_input = torch.cat([embedded, num_feat], dim=1)
        static_out = F.leaky_relu(self.static_net(static_input))
        
        # 应用线性层
        temporal_out = self.linear_blocks(x)
