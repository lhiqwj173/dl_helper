...
class DeepLOB_v2_ExtraLarge(nn.Module):
    """
    DeepLOB_v2_Large的超大容量版本 (参数量增加约33%)
    - 整体架构与Large版本保持一致。
    - 显著增加了各模块的通道数和隐藏维度以提升模型容量。
    - 参数量: 883738
    """
    def __init__(
            self,
        ...
        self.static_feature_processor = StaticFeatureProcessor(
            ...
            static_hidden=(100, 64), # 隐藏层加宽
            use_regularization=use_regularization,
        )

        # --- 2. LOB特征处理流 (增大) ---
        # Large: lob_conv_channels=40
        lob_conv_channels = 48 
        self.lob_conv1 = self._build_conv_block(1, lob_conv_channels, nn.LeakyReLU(negative_slope=0.01))
        self.lob_conv2 = self._build_conv_block(lob_conv_channels, lob_conv_channels, nn.Tanh())
        ...
        # Inception模块 (增大)
        # Large: inception_channels=80
        inception_channels = 96
        ...
        # LOB流的TCN (增大)
        # Large: channels=[160, 128, 96]
        lob_tcn_input_dim = inception_channels * 3
        lob_tcn_channels = [192, 160, 128] 
        ...
        # --- 3. 延伸特征处理流 (增大) ---
        # Large: channels=[48, 48]
        ext_tcn_channels = [64, 64] 
        ...
        # --- 4. 融合网络 (增大) ---
        # Large: fusion_input -> Linear(192, 96)
        fusion_input_dim = lob_tcn_output_dim + ext_tcn_output_dim + static_out_dim
        # fusion_input_dim = 128 + 64 + 64 = 256
        self.fusion_net = nn.Sequential(
            nn.Linear(fusion_input_dim, 128), # 隐藏层加宽
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Dropout(0.3) if self.use_regularization else nn.Identity(),
            nn.Linear(128, self.y_len)
        )
    
        self.apply(self._init_weights)

    # 辅助函数，使__init__更整洁
    def _build_conv_block(self, in_c, out_c, activation):
        return nn.Sequential(
            nn.Conv2d(in_channels=in_c, out_channels=out_c, kernel_size=(1,2), stride=(1,2)),
            nn.BatchNorm2d(out_c), activation,
            nn.Conv2d(in_channels=out_c, out_channels=out_c, kernel_size=(4,1)),
            nn.BatchNorm2d(out_c), activation,
            nn.Conv2d(in_channels=out_c, out_channels=out_c, kernel_size=(4,1)),
            nn.BatchNorm2d(out_c), activation,
        )
...
class test(test_base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        args = []
        for i in range(5):
            for model_cls in [DeepLOB_v2_ExtraLarge]:
