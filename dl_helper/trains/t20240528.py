...
class trainer(trainer_base):
    ...
    def init_param(self, data_folder=''):
        ...
        y_n = 3
        title = f'binctabl_p30_v{self.idx}'
        data_parm = {
        ...
        init_param(
            ...
            learning_rate=0.0001, batch_size=64, workers=0,

            # 数据增强
            random_scale=0.01, random_mask_row=0.5,

            # 3分类
            y_n=y_n, classify_y_idx=2,classify_func=lambda x:0 if x>0 else 1 if x<0 else 2,

            data_folder=data_folder,

            describe='binctabl predict_n=30'
        )
