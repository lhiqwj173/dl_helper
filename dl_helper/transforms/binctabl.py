...
class transform():
    def __init__(self, device, param, raw_time_length, scale_prob=0.005, min_scale=0.97, max_scale=1.03):
        ...
        # 用于random_mask_row
        self.rand_cols = torch.zeros(self.batch_size, self.time_length, dtype=torch.int64, device=self.device)
        print(self.rand_cols, self.rand_cols.device)
        self.rand_cols = self.rand_cols.to(self.device)
