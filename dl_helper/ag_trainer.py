...
from dl_helper.tool import output_leaderboard_html
...
def autogluon_train_func(quality='medium', title='', id='id', label='label', use_length=0, yfunc=None, train_data_folder=''):
    ...
    print(f'读取训练数据({((time.time() - begin_time) / 3600):2f}h)')
    ...
    print(f'训练数据长度: {len(train_data)}({((time.time() - begin_time) / 3600):2f}h)')
    # 预处理标签
    if not None is yfunc:
        train_data[label] = train_data[label].apply(yfunc)

    print(f'开始训练模型({((time.time() - begin_time) / 3600):2f}h)')
    ...
    print(f'读取测试数据({((time.time() - begin_time) / 3600):2f}h)')
    ...
    print(f'测试数据长度: {len(test_data)}({((time.time() - begin_time) / 3600):2f}h)')
    # 预处理标签
    if not None is yfunc:
        test_data[label] = test_data[label].apply(yfunc)

    print(f'储存预测测试数据集({((time.time() - begin_time) / 3600):2f}h)')
    ...
    print(f'评估模型({((time.time() - begin_time) / 3600):2f}h)')
    ...
    output_leaderboard_html(leaderboard, os.path.join(root, f'leaderboard.html'))

    print(f'停止报告线程({((time.time() - begin_time) / 3600):2f}h)')
    stop_flag = True
    thread.join()
