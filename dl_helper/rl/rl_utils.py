...
class GradientCompressor_0:
    ...
    def __init__(self, sparsity_ratio=0.1, quantize_bits=8, pad_value=None, min_elements=0):
        ...
        # 默认为 int(n * sparsity_ratio)>=1 的最小n
    ...
    def decompress(self, compressed_grads, compress_info):
        ...
        for idx, (quantized_values, info) in enumerate(zip(compressed_grads, compress_info)):
            try:
                ...
                if info.get('is_small_gradient', False):
                    ...
                    if valid_values.numel() > 0:
                        ...
                        if isinstance(indices, (int, np.integer)):
                            ...
                            # # 修改这里：将indices转换为元组形式的索引
                            # indices = torch.tensor(indices[:info['valid_size']], device=quantized_values.device)
                            # valid_indices = indices.view(-1)  # 将索引展平为一维
