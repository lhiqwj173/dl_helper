...
def run(
    ...
    if run_type == 'server':
        ...
        if need_val:
            # 绘制训练曲线
            plot_training_curve(train_title, train_folder, time.time() - begin_time, y_axis_max=6500)
        ...
        # 绘制训练曲线
        plot_training_curve(train_title, train_folder, out_file, time.time() - begin_time, custom_plotter=LobPlotter(), y_axis_max=6500)
        # 停止算法
        algo.stop()
        log(f"algo.stop done")
