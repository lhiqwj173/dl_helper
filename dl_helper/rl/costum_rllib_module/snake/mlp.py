...
class MLPEncoder(TorchModel, Encoder):
    def __init__(self, config):
        ...
        for hidden_size in self.hidden_sizes:
            fcs.append(nn.Linear(input_size, hidden_size))  # 输入层到隐藏层1
            input_size = hidden_size

        self.fcs = nn.ModuleList(fcs)
        self.fc = nn.Linear(input_size, self.output_dims[0])  # 隐藏层2到输出层
...
if __name__ == "__main__":
    net = MLPEncoderConfig(input_dims=(5, 5), hidden_sizes=[64, 64], _output_dims=8).build()
    ...
    batch_size = 2
    input_tensor = torch.randn(batch_size, 5*5)  # PyTorch格式
