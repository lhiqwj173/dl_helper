...
class SimpleEncoder(TorchModel, Encoder):
    def __init__(self, config):
        ...
        self.static_net = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Dropout(0.1),
            nn.Linear(self.extra_input_dims * 4, self.extra_input_dims * 8),
            nn.LayerNorm(self.extra_input_dims * 8),
            nn.LeakyReLU(),
            nn.Dropout(0.1),
            nn.Linear(self.extra_input_dims * 8, self.output_dims),
        )
    ...
    def _forward(self, inputs: dict, **kwargs) -> dict:
        ...
        # 静态特征处理
        static_out = self.static_net(extra_x)

        # 数值检查
        if torch.isnan(static_out).any() or torch.isinf(static_out).any():
            self.error_count += 1

        return {ENCODER_OUT: static_out}

@dataclass
class SimpleEncoderConfig(ModelConfig):
    """
...
class SimplePPOCatalog(PPOCatalog):
    """
    ...
    def _determine_components_hook(self):
        ...
        # 生成配置
        self._encoder_config = SimpleEncoderConfig(input_dims=input_dims, extra_input_dims=extra_input_dims, _output_dims=output_dims)
...
class SimpleIntrinsicCuriosityModel(IntrinsicCuriosityModel):
    def setup(self):
        ...
        # 生成配置
        self._feature_config = SimpleEncoderConfig(input_dims=input_dims, extra_input_dims=extra_input_dims, _output_dims=output_dims)
...
if __name__ == "__main__":
    net = SimpleEncoderConfig(input_dims=(10, 20), extra_input_dims=4, _output_dims=6).build()
    print(net)
