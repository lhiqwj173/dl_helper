...
# D-003：运行预算由独立 ExecutionPolicy 提供，不再出现在业务配置
...
def _torch_cfg(run_id, max_epochs=5):
...
def test_torch_budget_preempts_and_resumes(tmp_path):
    ...
    cfg1 = _torch_cfg("budget-torch", max_epochs=5)
    ...
    cfg2 = _torch_cfg("budget-torch", max_epochs=8)
...
def _skl_cfg(run_id, max_epochs=5):
    schema = default_schema()
...
def test_sklearn_incremental_budget_preempts(tmp_path):
    ...
    cfg = _skl_cfg("budget-skl", max_epochs=5)
    layout = RunLayout(run_dir)
    layout.ensure()
    exp = build_sklearn_experiment("experiments.sklearn_incremental:build_experiment", cfg.experiment)
    r = run_sklearn_worker_experiment(exp, cfg, Platform(), layout, resume="auto",
                                      budget_monotonic=_AdvancingClock(), execution_policy=_BUDGET)
